...
# THIS LAUNCHES BOTS WITHOUT THE WORLD FRAME.
...
def generate_launch_description():
    ...
    number_of_robots = 4
    namespace = 'BOT'
    # pose = [[-8.5, -8.5], [-4.5, -9.0], [-1.1, -6.4], [-8.5, -2.0]]
    pose = [[-8.5, -8.5], [-4.5, -9.0], [20.1, 20.4], [-8.5, -2.0]]
    model_folder = '/home/dejoy/Desktop/swam_clustering/src/multibot/models/turtlebot3_burger'
    urdf_path = os.path.join(model_folder, 'model.sdf')

    save_path = os.path.join(model_folder, 'tmp')
    
    launch_file_dir = os.path.join(get_package_share_directory('multibot'), 'launch')
    pkg_gazebo_ros = get_package_share_directory('gazebo_ros')
    multibot_pkg = get_package_share_directory('multibot')


    use_sim_time = LaunchConfiguration('use_sim_time', default='false')

    world = os.path.join(multibot_pkg, 'worlds', 'static_world_old.world')
